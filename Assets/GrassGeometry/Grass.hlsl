...
void geom(uint primitiveID : SV_PrimitiveID, triangle Varyings input[3], inout TriangleStream<GeometryOutput> triStream) {
	...
	if (_WindStrength != 0){
		...
		windMatrix = AngleAxis3x3(((wind+ _WindDirection) * PI).y, normalize(float3(wind.x, wind.x, wind.y) + _WindDirection));
	} else {
	...
	for (int i = 1; i < bladeSegments; i++) {
		...
		output.positionWS = positionWS + mul(transformMatrixWithWind, float3(w, b*_Height, h));
		output.positionCS = WorldToHClip(output.positionWS, normalWS);
	...
	output.positionWS = positionWS + mul(transformMatrixWithWind, float3(0, bend*_Height, height));
	output.positionCS = WorldToHClip(output.positionWS, normalWS);
